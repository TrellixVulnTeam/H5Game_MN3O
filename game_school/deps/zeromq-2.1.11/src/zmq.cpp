...
void* _defauilt_zmq_msg_alloc_fn(size_t size,void* hint)
{
	return malloc(size);
}

void _default_zmq_msg_free_fn_(void* ptr,size_t size,void* hint)
{
	free(ptr);
}

static zmq_msg_alloc_fn* g_zmq_msg_alloc_fn = &_defauilt_zmq_msg_alloc_fn;
static zmq_msg_free_fn* g_zmq_msg_free_fn = &_default_zmq_msg_free_fn_;
static void* g_zmq_msg_hint = 0;

int zmq_msg_memory_fn(zmq_msg_alloc_fn* fnalloc,zmq_msg_free_fn* fnfree,void* hint)
{
	g_zmq_msg_alloc_fn = fnalloc;
	g_zmq_msg_free_fn = fnfree;
	g_zmq_msg_hint = NULL;
	return 0;
}
...
int zmq_msg_init_size (zmq_msg_t *msg_, size_t size_)
{
    if (size_ <= ZMQ_MAX_VSM_SIZE) {
        ...
        msg_->content =
            (zmq::msg_content_t*) (*g_zmq_msg_alloc_fn) (sizeof (zmq::msg_content_t) + size_,g_zmq_msg_hint);
        if (!msg_->content) {
            errno = ENOMEM;
            return -1;
        }
...
{
    msg_->content = (zmq::msg_content_t*) (*g_zmq_msg_alloc_fn) (sizeof (zmq::msg_content_t),g_zmq_msg_hint);
    ...
    if (msg_->content != (zmq::msg_content_t*) ZMQ_DELIMITER &&
        ...
        if (!(msg_->flags & ZMQ_MSG_SHARED) || !content->refcnt.sub (1)) {
            ...
            if (content->ffn)
			{
				...
				(*g_zmq_msg_free_fn)(content,sizeof (zmq::msg_content_t),g_zmq_msg_hint);
			}
			else
			{
				(*g_zmq_msg_free_fn)(content,sizeof (zmq::msg_content_t) + content->size,g_zmq_msg_hint);
			}
        }
    }
