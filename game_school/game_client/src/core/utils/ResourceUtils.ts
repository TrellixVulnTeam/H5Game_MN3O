...
class ResourceUtils extends BaseClass {
    ...
    public loadGroups($groupName:string, $subGroups:Array<any>, $onResourceLoadComplete:Function, $onResourceLoadProgress:Function, $onResourceLoadTarget:any):void {
        let needLoadArr= this.IsGroupLoadedByLoadGroup($subGroups);
     ...
     */
    public pilfererLoadGroup($groupName:string, $subGroups:Array<any> = null,$onResourceLoadComplete:Function=null, $onResourceLoadProgress:Function=null, $onResourceLoadTarget:any=null):void {
        //添加前缀，防止与正常加载组名重复
        let needLoadArr= this.IsGroupLoadedByLoadGroup($subGroups);
        if(needLoadArr.length<0){
            if($onResourceLoadComplete!=null){
    ...
    private IsGroupLoadedByLoadGroup($subGroups=[])
    {
        var subGroups=[];
    ...
    private IsGroupLoadedByLoadResource($resources = [], $groups = [])
    {
