...
var ResourceUtils = (function (_super) {
    ...
    ResourceUtils.prototype.loadGroups = function ($groupName, $subGroups, $onResourceLoadComplete, $onResourceLoadProgress, $onResourceLoadTarget) {
        var needLoadArr = this.IsGroupLoadedByLoadGroup($subGroups);
     ...
     */
    ResourceUtils.prototype.pilfererLoadGroup = function ($groupName, $subGroups, $onResourceLoadComplete, $onResourceLoadProgress, $onResourceLoadTarget) {
    ...
    ResourceUtils.prototype.onResourceLoadError = function (event) {
        Log.Error(event.groupName + "资源组有资源加载失败");
    ...
    ResourceUtils.prototype.loadResource = function ($resources, $groups, $onResourceLoadComplete, $onResourceLoadProgress, $onResourceLoadTarget) {
        ...
        if ($onResourceLoadProgress === void 0) { $onResourceLoadProgress = null; }
        if ($onResourceLoadTarget === void 0) { $onResourceLoadTarget = null; }
        var needLoadArr = this.IsGroupLoadedByLoadResource($resources, $groups);
        if (needLoadArr.length > 0) {
    ...
    ResourceUtils.prototype.IsGroupLoadedByLoadGroup = function ($subGroups) {
        if ($subGroups === void 0) { $subGroups = []; }
        var subGroups = [];
        for (var i = 0; i < $subGroups.length; i++) {
            if (!RES.isGroupLoaded($subGroups[i])) {
                subGroups.push($subGroups[i]);
            }
        }
        return subGroups;
    };
    ResourceUtils.prototype.IsGroupLoadedByLoadResource = function ($resources, $groups) {
        if ($resources === void 0) { $resources = []; }
        if ($groups === void 0) { $groups = []; }
        var resourcesGroup = [], subGroups = [];
        var groupLoadedNum = 0, subGroupLoadedNum = 0;
        for (var i = 0; i < $resources.length; i++) {
            if (!RES.isGroupLoaded($resources[i])) {
                resourcesGroup.push($resources[i]);
            }
        }
        ...
        return resourcesGroup.concat(subGroups);
    };
