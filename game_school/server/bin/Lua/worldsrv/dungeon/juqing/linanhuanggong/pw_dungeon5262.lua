...
local WaveMaxTime = 30					-----刷新下一波最长时间
local MonterWveoNum = 0							----小怪波数
local MonterID = {66201,66202,}						----怪物ID
...
function CurrentDungeonScript:MonsterSpawn()
	...
	if(MonterWveoNum%2 ~= 0)then
		...
		self.SModDungeonScript:SpawnMultiMonster({"scn5262_xiaoguai_1",})---开刷新组
	else
		self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[2]))
		self.SModDungeonScript:SpawnMultiMonster({"scn5262_xiaoguai_2",})---开刷新组		
	end
