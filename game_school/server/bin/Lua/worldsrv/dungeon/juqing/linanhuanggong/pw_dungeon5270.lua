...
local NPCID = 67002						-----郭靖ID			-------不同难度的郭靖ID
local MonterMaxWaveNum = 5				-----最多刷新几波
local MonterNum = 41				----刷新小怪的总数-----如果改了每波刷新的怪物数量，这里必改
local WaveNum = {10,10,10,10,1}		----每波怪刷新的数量，这里的数量是在NpcSpawnMethodConfig.xls表里的-----如果改了每波刷新的怪物数量，这里必改
local WaveMaxTime = 40					-----刷新下一波最长时间
local MonterWaveNum = 0							----小怪波数
local MonterID = {67001,67003,}						----怪物ID
...
function CurrentDungeonScript:HumanEnterEvent(human) 
	...
	Dungeon_Stare = 1
	--self.SModDungeonScript:SpawnMultiMonster({"scn5270_1_xiaoguai",})---开刷新组
...
-- function CurrentDungeonScript:MonsterSpawn()
	...
	-- self.SModDungeonScript:SpawnMultiMonster({"scn5270_2_xiaoguai",})---开刷新组		
...
function CurrentDungeonScript:MonsterSpawn()---创建计时器，判断第几波，强刷
	...
	elseif(MonterWaveNum == 2)then
		self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[1]))
		self.SModDungeonScript:SpawnMultiMonster({"scn5270_3_xiaoguai",})---开刷新组
		MonterWaveNum = 3
	elseif(MonterWaveNum == 3)then
		self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[1]))
		self.SModDungeonScript:SpawnMultiMonster({"scn5270_4_xiaoguai",})---开刷新组
		MonterWaveNum = 4
	elseif(MonterWaveNum == 4)then
		self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[2]))
		self.SModDungeonScript:SpawnMultiMonster({"scn5270_boss",})---开刷新组
		MonterWaveNum = 5	
	end
