...
function CurrentDungeonScript:HumanEnterEvent(human) 
	...
	Dungeon_Stare = 1
	MonterWveoNum = 1
	self.SModDungeonScript:SpawnMultiMonster({"scn5149_1_xiaoguai",})---开刷新组
...
function CurrentDungeonScript:Timer1(human)
	...
	HuMan:GetModMotionMaster():ChangePosition({216.2,11.4,193.3}) ---解决播放剧本角色被重置位置问题.
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterWveoNum == 1)then
		...
		if(MonterNum[1] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5149_2_xiaoguai",})---开刷新组
			MonterWveoNum = 2
		end
	elseif(MonterWveoNum == 2)then
		...
		if(MonterNum[2] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5149_3_xiaoguai",})---开刷新组
			MonterWveoNum = 3
		end
	elseif(MonterWveoNum == 3)then
		...
		if(MonterNum[3] <= 0)then
			self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[2]))
			self.SModDungeonScript:SpawnMultiMonster({"scn5149_4_boss",})---开刷新组
			MonterWveoNum = 4
		end	
