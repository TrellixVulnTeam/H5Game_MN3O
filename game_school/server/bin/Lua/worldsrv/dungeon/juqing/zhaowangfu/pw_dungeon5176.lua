...
local MonterID = {37501,37502,37504}						----怪物ID
local MonterNum = {4,4,1,4,4,1}				----每波怪数量
local MonterTalkID = {50216,}		-----怪物进去战斗喊话
local MonterWveoNum = 0					----小怪波数
local DungeonConditionStr = {50247,50248,50249,}			------副本完成条件
local SecenStr = {50223,50223,}					-----副本提示文字
local MonterDieNum = 0					-----怪物死亡数量
local DungeonMaxTime = 180				-----副本最大时长
local SecenLaunch	= {140,}		----副本入场剧情ID 
----------------------------------------------------------副本信息列表
...
function CurrentDungeonScript:HumanEnterEvent(human) 
	...
	Dungeon_Stare = 1
	MonterWveoNum = 1
	self.SModDungeonScript:SpawnMultiMonster({"scn5176_1_xiaoguai",})---开刷新组
	...
	self.Stage:GetModDetour():EnableBlockEffects(5,false)
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterWveoNum == 1)then
		...
		if(MonterNum[1] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5176_2_xiaoguai",})---开刷新组
			MonterWveoNum = 2
		end
	elseif(MonterWveoNum == 2)then
		...
		if(MonterNum[2] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5176_3_boss",})---开刷新组
			MonterWveoNum = 3
		end
	elseif(MonterWveoNum == 3)then
		...
		if(MonterNum[3] <= 0)then
			self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[2]))
			self.SModDungeonScript:SpawnMultiMonster({"scn5176_4_xiaoguai",})---开刷新组
			MonterWveoNum = 4
		end	
	elseif(MonterWveoNum == 4)then
		...
		if(MonterNum[4] <= 0)then
			self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[2]))
			self.SModDungeonScript:SpawnMultiMonster({"scn5176_5_xiaoguai",})---开刷新组
			MonterWveoNum = 5
		end	
	elseif(MonterWveoNum == 5)then
		...
		if(MonterNum[5] <= 0)then
			self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[2]))
			self.SModDungeonScript:SpawnMultiMonster({"scn5176_6_boss",})---开刷新组
			MonterWveoNum = 6
		end			
...
function CurrentDungeonScript:Timer()
	...
	self.Stage:GetModDetour():EnableBlockEffects(5,true)	--打开剧情动画时暂时关闭的特效墙.
end
