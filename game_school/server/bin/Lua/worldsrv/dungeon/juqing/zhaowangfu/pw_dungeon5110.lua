...
local MonterID = {31201,31202}						----怪物ID
local MonterNum = {3,3,3,1}				----小怪数量
local MonterTalkID = {50074,50075,50076,}		-----怪物进去战斗喊话
local MonterWveoNum = 0					----小怪波数
local DungeonConditionStr = {50092,50093,}			------副本完成条件
local SecenStr = {50105,50106,}					-----副本提示文字
...
function CurrentDungeonScript:HumanEnterEvent(human) 
	...
	Dungeon_Stare = 1
	MonterWveoNum = 1
	self.SModDungeonScript:SpawnMultiMonster({"scn5110_1_xiaoguai_jinzhan",})---开刷新组
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterWveoNum == 1)then
		...
		if(MonterNum[1] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5110_2_xiaoguai_jinzhan",})---开刷新组
			MonterWveoNum = 2
		end
	elseif(MonterWveoNum == 2)then
		...
		if(MonterNum[2] <= 0)then
			self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[1]))
			self.SModDungeonScript:SpawnMultiMonster({"scn5110_3_xiaoguai_jinzhan",})---开刷新组
			MonterWveoNum = 3
		end
	elseif(MonterWveoNum == 3)then
		...
		if(MonterNum[3] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5110_4_boss",})---开刷新组
			MonterWveoNum = 4
		end
