...
local MonterTalkID = {50140,50141,50142,}		-----怪物进去战斗喊话
local DungeonConditionStr = {50127,}			------副本完成条件
local SecenStr = {50135,}					-----副本提示文字
...
local MonterID = {32601,}														----怪物ID
local OBJPos = {Pos = {42.2,16.4,66.5}, Org = {-0.5,0.0,-0.8}, ID = 32602}		-----梅超风
local WaveMaxTime = 20					-----刷新下一波最长时间
local WaveNum = {14,14,14,14,14}		----每波怪刷新的数量，这里的数量是在NpcSpawnMethodConfig.xls表里的-----如果改了每波刷新的怪物数量，这里必改
local MonterNum = 70				----刷新小怪的总数-----如果改了每波刷新的怪物数量，这里必改
...
function CurrentDungeonScript:MonsterSpawn()
	...
	self.SModDungeonScript:SpawnMultiMonster({"scn5127_xiaoguai",})---开刷新组		
