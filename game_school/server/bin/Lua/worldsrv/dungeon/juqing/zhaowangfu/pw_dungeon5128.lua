...
local DungeonConditionStr = {50128,50129,}			-----副本完成条件
local MoneterID = {32701,32702,32703,}		----怪物ID
local MonterNum = {3,3,1,3,3,1}						----每波怪的个数
local SecenStr = {50132,50136,50137,}					-----副本提示文字
----------------------------------------------------------副本信息列表
...
function CurrentDungeonScript:HumanEnterEvent(human) 
	...
	self.SModDungeonScript:SpawnMultiMonster({"scn5128_1_xiaoguai",})---开刷新组
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterWveoNum == 1)then
		...
		if(MonterNum[1] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5128_2_xiaoguai",})---开刷新组
			MonterWveoNum = 2
		end
	elseif(MonterWveoNum == 2)then
		...
		if(MonterNum[2] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5128_boss1",})---开刷新组
			MonterWveoNum = 3			
		end
	elseif(MonterWveoNum == 3)then
		...
		if(MonterNum[3] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5128_3_xiaoguai",})---开刷新组
			MonterWveoNum = 4			
		end	
	elseif(MonterWveoNum == 4)then
		...
		if(MonterNum[4] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5128_4_xiaoguai",})---开刷新组
			MonterWveoNum = 5			
		end	
	elseif(MonterWveoNum == 5)then
		...
		if(MonterNum[5] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5128_boss2",})---开刷新组
			MonterWveoNum = 6
		end	
