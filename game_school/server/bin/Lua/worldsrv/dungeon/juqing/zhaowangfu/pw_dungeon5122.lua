...
local SecenLaunch	= {102,106,}		----副本入场剧情ID 
...
local MoneterID = {32101,32102,32103,32104,}		----怪物ID
...
function CurrentDungeonScript:HumanEnterEvent(human)
	...
	self.SModDungeonScript:SpawnMultiMonster({"scn5122_1_xiaoguai",})---开刷新组
...
-----梁子翁死亡剧情
function CurrentDungeonScript:Timer2()
	self.SModDungeonScript:PlotClean()
	self:UpDateCondition(1,1,1,DungeonConditionStr[1])
	self.SModDungeonScript:SpawnMultiMonster({"scn5122_3_xiaoguai",})---开刷新组
	MonterWveoNum = 4	
end
-----倒计时结束副本开始
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterWveoNum == 1)then
		...
		if(MonterNum[1] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5122_2_xiaoguai",})---开刷新组
			MonterWveoNum = 2
		end
	elseif(MonterWveoNum == 2)then
		...
		if(MonterNum[2] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5122_boss1",})---开刷新组
			MonterWveoNum = 3			
		end
	elseif(MonterWveoNum == 3)then
		MonterNum[3] = MonterNum[3] - 1
		if(MonterNum[3] <= 0)then
			self.SModDungeonScript:LaunchPolt(SecenLaunch[2], 0,"Timer2")	----播放剧情		
		end	
	elseif(MonterWveoNum == 4)then
		MonterNum[4] = MonterNum[4] - 1
		if(MonterNum[4] <= 0)then	
			self.SModDungeonScript:SpawnMultiMonster({"scn5122_4_xiaoguai",})---开刷新组
			MonterWveoNum = 5			
		end	
	elseif(MonterWveoNum == 5)then
		...
		if(MonterNum[5] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5122_boss2",})---开刷新组
			MonterWveoNum = 6
		end	
