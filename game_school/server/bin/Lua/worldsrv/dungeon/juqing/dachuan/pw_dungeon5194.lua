...
local MonterID = {39401,39402}						----怪物ID
local MonterNum = {4,4,4,1}				----小怪数量
local MonterTalkID = {50217,50218,50216,}		-----怪物进去战斗喊话
local MonterWveoNum = 0					----小怪波数
local DungeonConditionStr = {50174,50175,}			------副本完成条件
local SecenStr = {50203,50204,}					-----副本提示文字
...
function CurrentDungeonScript:HumanEnterEvent(human) 
	...
	Dungeon_Stare = 1
	MonterWveoNum = 1
	self.SModDungeonScript:SpawnMultiMonster({"scn5194_1_xiaoguai",})---开刷新组
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterWveoNum == 1)then
		...
		if(MonterNum[1] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5194_2_xiaoguai",})---开刷新组
	...
	elseif(MonterWveoNum == 2)then
		...
		if(MonterNum[2] <= 0)then
			self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[1]))
			self.SModDungeonScript:SpawnMultiMonster({"scn5194_3_xiaoguai",})---开刷新组
			self.Stage:GetModObjs():SuspendCreaturesAITimed(1)----停止所有AI
			MonterWveoNum = 3
		end
	elseif(MonterWveoNum == 3)then
		...
		if(MonterNum[3] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5194_boss",})---开刷新组
			self.Stage:GetModObjs():SuspendCreaturesAITimed(1)----停止所有AI
			MonterWveoNum = 4
		end
