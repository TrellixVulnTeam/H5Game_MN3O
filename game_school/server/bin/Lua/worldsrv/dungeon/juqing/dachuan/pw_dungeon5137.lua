...
local MonterID = {33601,}						----怪物ID
local NpcID = {ID = {},Pos = {},Org = {},}						-----洪七公，郭靖ID,笼子
NpcID.ID[1] = 33603							-----洪七公
...
local SecenLaunch	= {120,}		----副本入场剧情ID 
local HumEnterPos = {430.1,75.1,205.9}	-----进入场景的点
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterWveoNum == 1)then
		...
		if(MonterNum[1] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5137_2_xiaoguai",})---开刷新组
			MonterWveoNum = 2
		end
	elseif(MonterWveoNum == 2)then
		...
		if(MonterNum[2] <= 0)then
			self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[1]))
			self.SModDungeonScript:SpawnMultiMonster({"scn5137_3_xiaoguai",})---开刷新组
			self.SModDungeonScript:SpawnMultiMonster({"scn5137_4_xiaoguai",})---开刷新组
			MonterWveoNum = 3
		end
...
function CurrentDungeonScript:Timer(human)
	...
	self.SModDungeonScript:SpawnMultiMonster({"scn5137_1_xiaoguai",})---开刷新组
