...
local MonterID = {33801,}						----怪物ID
local NpcID = {ID = {},Pos = {},Org = {},}						-----洪七公，郭靖ID,笼子
NpcID.ID[1] = 33803							-----洪七公
...
function CurrentDungeonScript:HumanEnterEvent(human) 
	...
	Dungeon_Stare = 1
	MonterWveoNum = 1
	self.SModDungeonScript:SpawnMultiMonster({"scn5139_1_xiaoguai",})---开刷新组
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterWveoNum == 1)then
		...
		if(MonterNum[1] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5139_2_xiaoguai",})---开刷新组
			MonterWveoNum = 2
		end
	elseif(MonterWveoNum == 2)then
		...
		if(MonterNum[2] <= 0)then
			self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[1]))
			self.SModDungeonScript:SpawnMultiMonster({"scn5139_3_xiaoguai",})---开刷新组
			self.SModDungeonScript:SpawnMultiMonster({"scn5139_4_xiaoguai",})---开刷新组
			MonterWveoNum = 3
		end
