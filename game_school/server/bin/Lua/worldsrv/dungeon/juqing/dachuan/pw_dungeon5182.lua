...
local MonterID = {38101,38102}						----怪物ID
local MonterNum = {4,4,4,1}				----小怪数量
local MonterTalkID = {50215,}		-----怪物进去战斗喊话
local MonterWveoNum = 0					----小怪波数
local DungeonConditionStr = {50166,50167,}			------副本完成条件
local SecenStr = {50195,50196,}					-----副本提示文字
local MonterDieNum = 0					-----怪物死亡数量
local DungeonMaxTime = 180				-----副本最大时长
local SecenLaunch	= {141,}		----副本入场剧情ID 
----------------------------------------------------------副本信息列表
...
function CurrentDungeonScript:HumanEnterEvent(human) 
	...
	Dungeon_Stare = 1
	MonterWveoNum = 1
	self.SModDungeonScript:SpawnMultiMonster({"scn5182_1_xiaoguai",})---开刷新组
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterWveoNum == 1)then
		...
		if(MonterNum[1] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5182_2_xiaoguai",})---开刷新组
	...
	elseif(MonterWveoNum == 2)then
		...
		if(MonterNum[2] <= 0)then
			self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[1]))
			self.SModDungeonScript:SpawnMultiMonster({"scn5182_3_xiaoguai",})---开刷新组
			self.Stage:GetModObjs():SuspendCreaturesAITimed(1)----停止所有AI
			MonterWveoNum = 3
		end
	elseif(MonterWveoNum == 3)then
		...
		if(MonterNum[3] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5182_boss",})---开刷新组
			self.Stage:GetModObjs():SuspendCreaturesAITimed(1)----停止所有AI
			MonterWveoNum = 4
...
function CurrentDungeonScript:Timer()
	...
	--self.Stage:GetModDetour():EnableBlockEffects(24,false)
	self.Stage:GetModDetour():EnableBlockEffects(23,true)  --动画拍到这个空气墙了.先取消战斗开始加载.
end
