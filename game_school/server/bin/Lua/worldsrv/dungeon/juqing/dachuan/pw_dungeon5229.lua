...
local DungeonConditionStr = {50194,}			------副本完成条件
local SecenStr = {50214,}					-----副本提示文字
local NPCPos = {446.3,75.5,203.1}						-----王处一坐标
local NPCOra = {-1,0.0,0}						-----王处一朝向
local DungeonMaxTime = 400				-----副本最大时长400
local NPCID = 62902						-----王处一ID			-------不同难度的王处一ID
local MonterMaxWveoNum = 5				-----最多刷新几波
local MonterNum = 60				----刷新小怪的总数-----如果改了每波刷新的怪物数量，这里必改
local WaveNum = {12,12,12,12,12}		----每波怪刷新的数量，这里的数量是在NpcSpawnMethodConfig.xls表里的-----如果改了每波刷新的怪物数量，这里必改
local WaveMaxTime = 40					-----刷新下一波最长时间
local MonterWveoNum = 0							----小怪波数
local MonterID = {62901,}						----怪物ID
...
function CurrentDungeonScript:MonsterSpawn()
	...
	self.SModDungeonScript:SpawnMultiMonster({"scn5229_1_xiaoguai",})---开刷新组		
