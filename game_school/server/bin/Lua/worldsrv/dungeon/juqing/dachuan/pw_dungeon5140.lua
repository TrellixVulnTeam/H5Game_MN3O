...
local MonterID = {33901,33902}						----怪物ID
local MonterNum = {3,3,1}				----小怪数量
local MonterTalkID = {50151,50152,}		-----怪物进去战斗喊话
local MonterWveoNum = 0					----小怪波数
local DungeonConditionStr = {50146,50147,}			------副本完成条件
local SecenStr = {50156,50157,}					-----副本提示文字
...
local SecenLaunch	= {123,}		----副本入场剧情ID 
local HumEnterPos = {420.4,65.8,167.4}	-----进入场景的点
...
function CurrentDungeonScript:HumanEnterEvent(human) 
	self.SModDungeonScript:LaunchPolt(SecenLaunch[1],0, "Timer")	----播放剧情
...
function CurrentDungeonScript:MonterEnterCombat(creature)
	...
	if(Mcreature ~= nil)then
		...
		local CreatureTypeID = Mcreature:GetCreatureType()
		if(CreatureTypeID == MonterID[#MonterID])then
			if(rand <= 50)then
				Mcreature:Talk(50160)
			else
				Mcreature:Talk(50161)
			end
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterWveoNum == 1)then
		...
		if(MonterNum[1] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5140_2_xiaoguai",})---开刷新组
			MonterWveoNum = 2
		end
	elseif(MonterWveoNum == 2)then
		...
		if(MonterNum[2] <= 0)then
			...
			self.Stage:GetModDetour():EnableBlockEffects(21,false)
			self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,tostring(SecenStr[2]))
			self.SModDungeonScript:SpawnMultiMonster({"scn5140_boss",})---开刷新组
			MonterWveoNum = 3
		end
...
-----倒计时结束副本开始
function CurrentDungeonScript:Timer(human)	
	if(human == nil)then
		return
	end
	local HuMan= human:cast('Human')	
	self.time9 = self.SModDungeonScript:CreateTimer(DownTime,"Timer1")
	self.Stage:GetModCommon():BroadcastClientOperation(UI_CHALLENGE_TIMER,tostring(DownTime))
	self.SModDungeonScript:SpawnMultiMonster({"scn5140_1_xiaoguai",})---开刷新组
	self.Stage:GetModObjs():SuspendCreaturesAI()----停止所有AI
	HuMan:GetModMotionMaster():ChangePosition(HumEnterPos)
	self.SModDungeonScript:PlotStart()
	self.SModObjs:Broadcast(msgids.SCDungeonTargetList, msg)
end
