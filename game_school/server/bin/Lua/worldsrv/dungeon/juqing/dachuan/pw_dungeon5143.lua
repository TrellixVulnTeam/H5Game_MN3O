...
local MonterID = {34201,34202}						----怪物ID
local MonterNum = {3,3,1}				----小怪数量
local MonterTalkID = {50151,50152,}		-----怪物进去战斗喊话
local MonterWveoNum = 0					----小怪波数
local DungeonConditionStr = {50148,50149,}			------副本完成条件
local SecenStr = {50156,50158,}					-----副本提示文字
...
local SecenLaunch	= {121,}		----副本入场剧情ID 
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterWveoNum == 1)then
		...
		if(MonterNum[1] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5143_2_xiaoguai",})---开刷新组
			MonterWveoNum = 2
		end
	elseif(MonterWveoNum == 2)then
		...
		if(MonterNum[2] <= 0)then
			...
			self.SModDungeonScript:SpawnMultiMonster({"scn5143_boss",})---开刷新组
			MonterWveoNum = 3
		end
...
function CurrentDungeonScript:Timer(human)
	...
	self.SModDungeonScript:SpawnMultiMonster({"scn5143_1_xiaoguai",})---开刷新组
