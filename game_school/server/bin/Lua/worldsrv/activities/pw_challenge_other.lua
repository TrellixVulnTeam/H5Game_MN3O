...
pwngs_require("./Lua/worldsrv/activities/pw_challenge_otherData.lua")
...
function CurrentDungeonScript:HumanEnterEvent(human) 
	...
	MaxLevel = self.MainHuman:GetModChallengeOther():GetMaxLevel()----获取策划设置的最高层
	CurrentLevel = self.MainHuman:GetModChallengeOther():GetCurrLevel()----获取玩家最高成功挑战层数
	if(CurrentLevel == nil or CurrentLevel < 1)then
		return
	end
	local HumPos = ChallengeOther.MonsterTable[CurrentLevel]["hum_pos"]
	if(HumPos == nil)then
		self.MainHuman:GetModMotionMaster():ChangePosition(ChallengeOther.MonsterPosition[1][1])---设置玩家进入场景的坐标点
		self.MainHuman:GetModMotionMaster():ChangeOrientation(ChallengeOther.MonsterPosition[1][2])---设置玩家进入场景的朝向
	end

	local pos_info = ChallengeOther.MonsterPosition[HumPos]
	if(HumPos ~=nil)then
	...
	if(CurrentLevel <= MaxLevel)then
		...
		self.MainHuman:GetModChallengeOther():OnStart()	--通知服务器刷新场景
...
----------给客户端发送时间
	local Wtime = ChallengeOther.MonsterTable[CurrentLevel]["winTime"]	----每层的时间变量
	if(Wtime == nil)then
	...
	if(Wtime > 0 and CurrentLevel >= 1)then
		...
		self.MainHuman:SendMsg(msgids.SCChallengeOtherInfo,msg)		---给客户端发送时间
...
function CurrentDungeonScript:OnFightBegin()	--进场XX时间后开始刷怪
	...
	local Wtime = ChallengeOther.MonsterTable[CurrentLevel]["winTime"]
...
--pwdist.log("开始刷怪")
	...
	local win_type = ChallengeOther.MonsterTable[CurrentLevel]["win_type"]
	if(win_type == nil)then
		self.Stage:GetModCommon():BroadcastClientOperation(UI_WARN,4016)
		return
	end
	if(win_type == ChallengeOther.WIN_TYPE_KILLALL)then	-----根据刷怪类型确定刷怪方式
		self:SpawnCommonCreature(ChallengeOther.MonsterTable[CurrentLevel]["monster_table"][1])	
	elseif(win_type == ChallengeOther.WIN_TYPE_KILLBYWAVE)then	
		self:SpawnCommonCreature(ChallengeOther.MonsterTable[CurrentLevel]["monster_table"][1])
		local timeLength = ChallengeOther.MonsterTable[CurrentLevel]["time_nodes"]
	...
	for k = 1, #param,1 do
			...
			for i = 1, creature_cnt, 1 do
				if(#creature_poslist > 0)then
					local pos_info = ChallengeOther.MonsterPosition[creature_poslist[j]]
...
--pwdist.log("当前是多波怪层，刷新第"..CurrWave.."波怪")	
	local win_type = ChallengeOther.MonsterTable[CurrentLevel]["win_type"]
	if(win_type == ChallengeOther.WIN_TYPE_KILLALL )then	
		return
	end
	if(CurrWave <= ChallengeOther.MonsterTable[CurrentLevel]["win_param"])then
		self:SpawnCommonCreature(ChallengeOther.MonsterTable[CurrentLevel]["monster_table"][CurrWave])
		local timeLength = ChallengeOther.MonsterTable[CurrentLevel]["time_nodes"]
...
function CurrentDungeonScript:Monster_Die(creature,killer)
	...
	if(MonterNum <= 0 and CurrentLevel <= MaxLevel)then
		...
		if(win_type == ChallengeOther.WIN_TYPE_KILLALL)then		
			...
			self.MainHuman:GetModChallengeOther():OnEnd(1)-----副本成功
...
--pwdist.log("取消本层波数计时器")
	...
	self.MainHuman:GetModChallengeOther():OnEnd(0)-----副本失败
	self.MainHuman:CloseAutoCombat()-----关闭自动战斗
	self.SModDungeonScript:CreateTimer(9,"Leave")	------X秒自动传送玩家离开场景
