...
namespace pwutils
{
	...
	namespace time
	{
		...
		extern std::string gmt_str()
		{
			tm tmcurr;
			time_t t = pwutils::time::clib_time();
			char daytime[100];
			...
			gmtime_r(&t,&tmcurr);
			asctime_r(&tmcurr, daytime);

			pch = strtok_r(daytime, " ",&outer_ptr);
			int i = 0;
			while(pch != NULL)
			{
				switch(i)
			...
			sprintf(result, "%s, %s %s %s %s GMT", week, realday, mon, realyear, time);
		...
		int64 asYYYYMMDD(int64 t)
		{
			...
			return (1900 + tmcurr.tm_year) * 10000 + (tmcurr.tm_mon +1) * 100 + tmcurr.tm_mday;
		}
		...
		int64 mktimeofoneday_YYYYMMDD(int64 yyyymmdd,int hour,int minute,int second)
		{
			int32 yyyy = static_cast<int32>(yyyymmdd / 10000);
			int32 mm = static_cast<int32>((yyyymmdd % 10000) / 100);
			int32 dd = static_cast<int32>(yyyymmdd % 100);

			char timebuf[100] = "";
			sprintf(timebuf,"%d-%d-%d %d:%d:%d",yyyy,mm,dd,hour,minute,second);

			return ptime(timebuf);
		}
		...
		int64 mktimeYYMMDD(int64 delta)
		{
			time_t curr;
			struct tm tmcurr = {0};
		...
		int64 mktimeNewYYMMDD(int64 delta)
		{
			time_t curr;
			struct tm tmcurr = { 0 };
