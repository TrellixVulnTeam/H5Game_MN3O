...
namespace pwutils
{
	namespace time
	{
		...
		extern int64 asYYYYMMDD(int64 t);
		...
		extern int64 mktimeofoneday_YYYYMMDD(int64 yyyymmdd,int hour = 0,int minute = 0,int second = 0);
		...
		extern int64 mktimeYYMMDD(int64 delta = 0);
		extern int64 mktimeNewYYMMDD(int64 delta = 0);
		...
		extern tm		tmdate();
		extern tm		tmdate(int64 t);

		// 获取某年某月中的总天数
		extern int get_days_in_month(int year, int month);
	}
}
