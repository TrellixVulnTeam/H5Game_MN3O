...
namespace gdb
{
	...
	int Client::OnDataIncoming()
	{
		...
		// 检查大小
		if(m_mReader.Length() > 128*1024*1024)
		{
			m_mReader.Skip(m_mReader.Length());
			m_mReader.Free(0);
			std::cerr << pwutils::time::str() << " " << __FUNCTION__ << ":" << __LINE__ << " Closed" << std::endl;
...
#else
		int64 cst_timeout = pwutils::time::Msec(8000);
#endif
	...
	{
#ifdef _DEBUG
		this->_timeoutTick = pwutils::time::get() + pwutils::time::Second(100);
#else
	...
	void Client::Response::CheckedBuffer(size_t i,std::string& rdata)
	{
		char* data = 0;
		size_t size = 0;
		_checked_buffer(&packets[0],packets.size(),i,&data,&size);
