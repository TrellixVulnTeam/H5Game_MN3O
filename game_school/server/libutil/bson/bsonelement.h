...
namespace mongo {
    ...
    */
    ...
    inline long long BSONElement::safeNumberLong() const {
        double d;
#ifdef _MSC_VER
		double dmax = (double)(_LLONG_MAX);
		double dmin = (double)(-_LLONG_MAX - _C2);
#else
		double dmax = (double)std::numeric_limits<long long>::max();
		double dmin = (double)std::numeric_limits<long long>::min();
#endif
        ...
        case NumberDouble:
            ...
            if ( d > dmax ){
                return (long long)dmax;
            }
            if ( d < dmin ){
                return (long long)dmin;
            }
        default:
            return numberLong();
        }
