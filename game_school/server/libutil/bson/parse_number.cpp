            ...
            if (limits::is_signed) {
                for (size_t i = 0; i < str.size(); ++i) {
                    ...
                    if (int(digitValue) >= base)
                        return Status(0 - __LINE__, "Bad digit");
...
#pragma warning(pop)
                        ...
                        return Status(0 - __LINE__, "Underflow");
                    }
                ...
                return Status(0 - __LINE__, "Negative value");
            }
        }
        else {
            for (size_t i = 0; i < str.size(); ++i) {
                ...
                if ((NumberType(limits::max() / base) < n) ||
                    ...
                    return Status(0 - __LINE__, "Overflow");
                }
...
namespace {
    ...
    Status parseNumberFromStringWithBase<double>(const StringData& stringValue,
        ...
        if (stringValue.empty())
            return Status(0 - __LINE__, "Empty string");

        if (isspace(stringValue[0]))
            return Status(0 - __LINE__, "Leading whitespace");
        ...
        if (endp != stringValue.size() + cStr) {
            ...
            return Status(0 - __LINE__, "Did not consume whole number.");
        }
        if (actualErrno == ERANGE)
            return Status(0 - __LINE__, "Out of range");
        *result = d;
        return Status::OK();
    }
