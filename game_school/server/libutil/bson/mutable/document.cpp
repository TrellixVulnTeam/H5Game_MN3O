...
#include <limits>
#include <vector>
#include "bson/mutable/damage_vector.h"
...
//         BOOST_STATIC_ASSERT(sizeof(ElementRep) == 32);
        ...
        Status getAttachmentError(const ElementRep& rep) {
            if (rep.sibling.left != Element::kInvalidRepIdx)
                return Status(0 - __LINE__, "dangling left sibling");
            if (rep.sibling.right != Element::kInvalidRepIdx)
                return Status(0 - __LINE__, "dangling right sibling");
            if (rep.parent != Element::kInvalidRepIdx)
                return Status(0 - __LINE__, "dangling parent");
            return Status(0 - __LINE__, "cannot add the root as a child");
        }
     ...
     *  Document.
     */
	class Document::Impl : public boost::noncopyable {
    ...
    Status Element::remove() {
        ...
        if (thisRep.parent == kInvalidRepIdx)
            return Status(0 - __LINE__, "trying to remove a parentless element");
        impl.disableInPlaceUpdates();
...
//     }
    ...
    Status Element::setValueBSONElement(const BSONElement& value) {
        ...
        if (value.type() == mongo::EOO)
            return Status(0 - __LINE__, "Can't set Element value to EOO");
    ...
    Status Element::setValue(const Element::RepIdx newValueIdx) {
        ...
        if (_repIdx == kRootRepIdx)
            return Status(0 - __LINE__, "Cannot call setValue on the root object");
