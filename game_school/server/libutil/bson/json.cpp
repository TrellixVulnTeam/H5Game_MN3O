...
#define JOPTIONS "gims"
    ...
    Status JParse::parseError(const StringData& msg) {
        ...
        ossmsg << " of:";
        ossmsg << _buf;
        return Status(0 - __LINE__, ossmsg.str());
    }
    ...
    Status JParse::object(const StringData& fieldName, BSONObjBuilder& builder, bool subObject) {
        ...
        if (firstField == "$oid") {
			...
			boost::scoped_ptr<BSONObjBuilder> subObjBuilder;
            if (subObject) {
...
//         }
		...
		const std::string& binData = pwutils::base64_decode(binDataString.c_str(),binDataString.length());
        if (!readToken(COMMA)) {
            return parseError("Expected ','");
        }
    ...
    Status JParse::dateObject(const StringData& fieldName, BSONObjBuilder& builder) {
		return Status(0 - __LINE__,"unsupport date");
//         if (!readToken(COLON)) {
//             return parseError("Expected ':'");
//         }
//         errno = 0;
...
//         return Status::OK();
    ...
    Status JParse::numberLong(const StringData& fieldName, BSONObjBuilder& builder) {
        ...
        // we know ahead of time where the number ends, which is not currently the case.
#ifdef _MSC_VER
        int64_t val = _strtoi64(_input,&endptr,10);
#else
...
#ifdef _MSC_VER
		retll = _strtoi64(_input,&endptrll,10);
#else
...
//     }
    ...
    BSONObj fromjson(const char* jsonString, int* len) {
        ...
        try {
            ...
            massert(17031, message.str(),false);
        }

        if (ret != Status::OK()) {
			std::ostringstream message;
            message << "code " << (int)ret.code() << ": " << ret.codeString() << ": " << ret.reason();
            massert(16619, message.str(),false);
        }
