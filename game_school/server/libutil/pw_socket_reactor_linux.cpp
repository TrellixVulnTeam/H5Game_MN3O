...
namespace pwutils
{
	...
	int SocketReactorImplEPOLL::Add( SocketConnection* connection,int mask )
	{
		int event = 0;
		if(mask & MASK_READ)
			event |= EPOLLIN;
	...
	int SocketReactorImplEPOLL::Del( SocketConnection* connection )
	{
		epoll_event ev;
		ev.events = 0;
		ev.data.ptr = connection;
	...
	int SocketReactorImplEPOLL::Mod( SocketConnection* connection,int mask )
	{
		int event = 0;
		if(mask & MASK_READ)
			event |= EPOLLIN;
	...
	int SocketReactorImplEPOLL::Update()
	{
		...
		nfds = epoll_wait(m_nEpoll, m_pEvents, m_nMaxSockets, 0);
		...
		{
			bool closed = false;
			SocketConnection* conn = (SocketConnection*)m_pEvents[i].data.ptr;
