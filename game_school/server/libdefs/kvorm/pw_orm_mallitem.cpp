...
namespace orm
{
	...
	std::string MallItem::getkey()
	{
		return this->rawkey();
	}

	std::string MallItem::getmeta()
	{
		return MallItem::meta();
	}

	void MallItem::set_autoincr(int64 key)
	{
		pwassert(false);
	}

	MallItem* MallItem::clone() const
	{
		bson::bo __obj;
		to_bson(__obj);
		MallItem* tmp = new MallItem();
		tmp->from_bson(__obj);
		return tmp;
	}

	void MallItem::to_snappy_buffer(std::string& __buf) const
	{
		bson::bo __obj;
		to_bson(__obj);
	...
	void MallItem::to_bson(std::string& __buf) const
	{
		bson::bo __obj;
		to_bson(__obj);
	...
	void MallItem::to_bson(bson::bo& __obj) const
	{
	...
	void MallItem::from_snappy_buffer(const std::string& __inbuf)
	{
		std::string tmpbuf;
	...
	void MallItem::from_snappy_buffer(const char* __data,size_t __size)
	{
		std::string tmpbuf;
	...
	void MallItem::from_bson(const char* __data,size_t __size)
	{
		pwassert(__data != 0);
	...
	void MallItem::from_bson(const bson::bo& __obj)
	{
	...
	int64 MallItem::hash_make(int64 seed)
	{
		int64 _result = seed;
	...
	int32 MallItem::get_cfg_id() const
	{
		return cfg_id;
	}

	void MallItem::set_cfg_id(int32 value)
	{
		this->cfg_id = value;
	}

	int32 MallItem::get_sold_count() const
	{
		return sold_count;
	}

	void MallItem::set_sold_count(int32 value)
	{
		this->sold_count = value;
	}


	bool MallItem::is_default_value() const
	{
		if(cfg_id != 0)
			return false;
