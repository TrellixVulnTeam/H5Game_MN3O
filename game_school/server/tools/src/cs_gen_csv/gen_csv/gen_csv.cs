...
namespace gen_csv
{
    public partial class gen_csv : Form
    {
        ...
        public gen_csv()
        {
            ...
            GetPrivateProfileString("ServerPathInfo", "Path", "..\\..\\server", temp, 255, ".\\gen_csv.ini");
...

            {
                ...
                GetPrivateProfileString("ConfigInfo", "BuildCpp", "false", strRead, 255, ".\\gen_csv.ini");

                if (strRead.ToString() == "true")
                {
                    cbBuildCPP.Checked = true;
                }
                else
                {
                    cbBuildCPP.Checked = false;
                }
            }
        ...
        private bool m_bBuildCpp = false;
        ...
        private void button_Click(object sender, EventArgs e)
        {
            ...
            {
                // 载入BuildCpp按钮状态
                m_bBuildCpp = cbBuildCPP.Checked;

                if (cbBuildCPP.Checked)
                {
                    WritePrivateProfileString("ConfigInfo", "BuildCpp", "true", ".\\gen_csv.ini");
                }
                else
                {
                    WritePrivateProfileString("ConfigInfo", "BuildCpp", "false", ".\\gen_csv.ini");
                }
            }
...
//                 }
            ...
            pro.StartInfo.FileName = ".\\csv_svn.bat";
        ...
        void Generate(string name, string file, string hpp_file, string cpp_file, string csv_file)
        {
            ...
            for (int i = 0; i < w.getNumberOfSheets(); ++i)
            {
                ...
                if (i == 0)
                {
                    if (IsServerNeedFile(s, name) == false)
                    {
                        writer.Close();
                        stream.Close();
        ...
        bool IsServerNeedFile(Sheet sheet, string name)
        {
            string cstype = GetStringFromCell(sheet, cst_RowForSide, 0, name);

            if (cstype.Equals("c") || cstype.Equals("C"))
                return false;
        ...
        void GenerateHpp(Sheet sheet,string hpp_file,string name)
        {
            ...
            W2F(0, writer, "");
                ...
                for (int i = 0; i < sheet.getColumns(); ++i)
                {
                    ...
                    string cstype = GetStringFromCell(sheet, cst_RowForSide, i, name);
            ...
            W2F(0, writer, "{");
                ...
                W2F(1, writer, "{");
                    ...
                    for (int col = 0; col < sheet.getColumns(); ++col)
                    {
                        ...
                        string cstype = GetStringFromCell(sheet, cst_RowForSide, col, name);
