...
void Sample_TileMesh::handleTools()
{
	...
	if (imguiCheck("Pwngs SetFlags", type == TOOL_SET_FLAGS))
	{
		setTool(new SetFlagsTool);
	}
	imguiSeparatorLine();
...
void Sample_TileMesh::handleRender()
{
		 ...
		 m_drawMode == DRAWMODE_NAVMESH_INVIS))
	{
		...
		duDebugDrawNavMeshPolysWithFlags(&dd, *m_navMesh, SAMPLE_POLYFLAGS_PWNGS_BLOCK1, duRGBA(200,0,0,128));
		duDebugDrawNavMeshPolysWithFlags(&dd, *m_navMesh, SAMPLE_POLYFLAGS_PWNGS_BLOCK2, duRGBA(0,200,0,128));
		duDebugDrawNavMeshPolysWithFlags(&dd, *m_navMesh, SAMPLE_POLYFLAGS_PWNGS_BLOCK3, duRGBA(0,0,200,128));
		duDebugDrawNavMeshPolysWithFlags(&dd, *m_navMesh, SAMPLE_POLYFLAGS_PWNGS_BLOCK4, duRGBA(200,0,200,128));
		duDebugDrawNavMeshPolysWithFlags(&dd, *m_navMesh, SAMPLE_POLYFLAGS_PWNGS_BLOCK5, duRGBA(0,200,200,128));
...
unsigned char* Sample_TileMesh::buildTileMesh(const int tx, const int ty, const float* bmin, const float* bmax, int& dataSize)
{
	...
	if (m_cfg.maxVertsPerPoly <= DT_VERTS_PER_POLYGON)
	{
		...
		for (int i = 0; i < m_pmesh->npolys; ++i)
		{
			...
			else if (m_pmesh->areas[i] == SAMPLE_POLYAREA_BLOCK1)
			{
				m_pmesh->flags[i] = SAMPLE_POLYFLAGS_PWNGS_BLOCK1;
			}
			else if (m_pmesh->areas[i] == SAMPLE_POLYAREA_BLOCK2)
			{
				m_pmesh->flags[i] = SAMPLE_POLYFLAGS_PWNGS_BLOCK2;
			}
			else if (m_pmesh->areas[i] == SAMPLE_POLYAREA_BLOCK3)
			{
				m_pmesh->flags[i] = SAMPLE_POLYFLAGS_PWNGS_BLOCK3;
			}
			else if (m_pmesh->areas[i] == SAMPLE_POLYAREA_BLOCK4)
			{
				m_pmesh->flags[i] = SAMPLE_POLYFLAGS_PWNGS_BLOCK4;
			}
			else if (m_pmesh->areas[i] == SAMPLE_POLYAREA_BLOCK5)
			{
				m_pmesh->flags[i] = SAMPLE_POLYFLAGS_PWNGS_BLOCK5;
			}
			else if(m_pmesh->areas[i] == SAMPLE_POLYAREA_DISABLE)
			{
