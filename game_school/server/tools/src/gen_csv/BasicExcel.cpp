...
size_t Workbook::Read(const char* data,size_t remainsize)
{
	size_t bytesRead = 0;
	short code;
	short lastcode;
...
//				break;
			...
			default:
				Record rec;
				int count = rec.Read(data+bytesRead);
				// bytesRead += rec.Read(data+bytesRead);
				if((bytesRead + count) >= remainsize)
					printf("12121");
...
size_t Workbook::SharedStringTable::Read(const char* data)
{
	...
	else
	{
		...
		for (size_t i=0, c=0; i<uniqueStringsTotal_; ++i)
		{
			...
			/*
			if (c != 0 && (c >= maxContinue || npos+stringSize*multiplier+3 <= continueIndices_[c]))
			{
			...
			else
			{
				...
				if (stringSize>0)
				{
					//bytesRead += strings_[i].Read(&*(data_.begin())+npos);
	...
	while (code == CODE::BOF)
	{
		...
		switch (bof.type_)
		{
			case WORKBOOK_GLOBALS:
				bytesRead += workbook_.Read(data+bytesRead,dataSize - bytesRead);
				break;
