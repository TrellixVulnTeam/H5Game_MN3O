...
def pack_msg_dir(src_dir, fact_dir, handler_dir, helper_dir):
    defFilePath = os.path.join( src_dir, 'MsgDef.ts' )
    print(defFilePath)
    if os.path.isfile( defFilePath ):
        hlist,helist = parsecs( defFilePath )
        msghlname = os.path.join(handler_dir, "MsgCommonHandler.ts")
        msghename = os.path.join(helper_dir, "MsgCommonHelper.ts")

        force_del(msghlname)
        force_del(msghename)

        txt = ""
        txt += exp_handler_common_file(msghlname, os.path.join(MAKO_PATH, GAME_MSG_BEGIN_MAKO_TS),"","")
        for file in hlist:
            class_name = file+"Handler"
            code_file = class_name + '.ts'
            code_path = os.path.join(handler_dir, code_file)
            mako_path = os.path.join(MAKO_PATH, GAME_MSG_COMMON_HANDLER_MAKO_ts)
            txt += exp_handler_common_file(msghlname, mako_path, class_name,file)
            if not isFileExist(code_path):
                # 不要覆盖已有的文件
                mako_path = os.path.join(MAKO_PATH, GAME_MSG_HANDLER_MAKO_TS)
        ...
        txt = ""
        txt += exp_handler_common_file(msghename, os.path.join(MAKO_PATH, GAME_MSG_BEGIN_MAKO_TS),"","")
        for file in helist:
            class_name = file+"Helper"
            code_file = class_name + '.ts'
            code_path = os.path.join(helper_dir, code_file)
            mako_path = os.path.join(MAKO_PATH, GAME_MSG_COMMON_HELPER_MAKO_TS)
        ...
        exp_mgr_code_file(os.path.join(fact_dir, "MsgHandlerFact.ts"),
                os.path.join(MAKO_PATH, GAME_MSG_FACT_MAKO_TS),
                hlist)
