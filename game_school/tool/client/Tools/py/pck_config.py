...
#创建表格build出来的文件  表格地址 bytes地址 代码地址
def pack_config_dir(src_dir, code_dir, bin_dir, csv_dir):
	#清除原来的文件
	clear_folder_type(code_dir, ".ts")
	clear_folder_type(bin_dir,".bytes")

	cls_name_list = []
	for file in os.listdir(src_dir):#遍历表格
		if os.path.isfile(os.path.join(src_dir, file)):#如果是文件或者是文件
			if os.path.splitext(file)[1] == '.xls':#表格xls格式
				file_name = os.path.splitext(file)[0]#表格名称
				class_name = file_name
				code_file = file_name + '.ts'#代码
				bin_file = file_name + '.bytes'#二进制
				cfg_data = parse_xls(os.path.join(src_dir, file))#获取表格数据
				if isclientxls(cfg_data):#是否为客户端用

					exp_bin_file(os.path.join(bin_dir, bin_file), cfg_data, class_name)#生成二进制
					exp_code_file(os.path.join(code_dir, code_file),os.path.join(MAKO_PATH, GAME_CONF_MAKO_TS),cfg_data, class_name, bin_file)#生成代码
					cls_name_list.append(class_name)#文件名列表
			else:
...
#	print "conffact:",cls_name_list
	exp_mgr_code_file(os.path.join(code_dir, "ConfFact.ts"),os.path.join(MAKO_PATH, GAME_CONF_FACT_MAKO_TS),cls_name_list)#生成conffact
